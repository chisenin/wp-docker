...
**文件: `wordpress-project/.github/workflows/version-monitor.yml`**
版本监测工作流，每天0点自动运行：检查 Alpine、PHP、Nginx 和 Composer 的最新版本，更新版本文件，并在检测到新版本时触发构建工作流。

**文件: `wordpress-project/.github/workflows/build-and-push.yml`**
构建与推送工作流，在推送至 `main` 或 PR 时触发，或由版本监测工作流自动触发。实现了按需构建功能，只构建发生变更的组件，提高构建效率。支持多平台构建（linux/amd64, linux/arm64）。
...
jobs:
  build:
    ...
    steps:
      ...
      - name: 检查变更的组件
        ...
        run: |
          ...
          if [ -f updated_components.txt ]; then
            echo "从 updated_components.txt 读取变更组件"
            updated_components=$(cat updated_components.txt)
          else
            echo "通过 git diff 检测变更组件"
            git fetch origin main
            # 检查 Dockerfiles/php, Dockerfiles/nginx, Dockerfiles/base 目录的变更
      ...
      - name: 构建并推送 Base 镜像
        if: contains(steps.check-changes.outputs.updated_components, 'base')
        run: |
          echo "::group::构建并推送 Base 镜像"
          docker buildx build --platform linux/amd64,linux/arm64 \
            -f ./Dockerfiles/base/Dockerfile \
